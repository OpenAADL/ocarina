...
package body Ocarina.Transfo.Fusions is
   ...
   is
      ...
      SC          : Node_Id;
   begin
      if not Is_Empty (Subcomponents (Old_Process)) or else
        Is_Empty (Connections (Old_Process)) then
      ...
      while Present (S) loop
         ...
         while Present (C) loop
            declare
               ...
               Src        : constant Node_Id := Corresponding_Entity
                 (Item (First_Node (Path (Source (C)))));
               Cnx_Number : constant Name_Id := Build_Unique_Name
                 (Connections (New_Process), Connection_Prefix);
            ...
            begin
               if Kind (Dst) = K_Port_Spec then
                  ...
                  if Dst_Item /= Entity (Entity_Ref (T1))
                    ...
                    (Entity (Entity_Ref (Src))) = CC_Data
                    and then Entity (Entity_Ref (Src)) = S then
                     Copy := True;
                     ...
                     if Local_Change then
                        declare
                           N : constant Name_Id := Get_String_Name
                             (Get_Name_String (Name (Identifier (Source (C))))
                              & "_n");
                        begin
                           E := New_Node (K_Entity_Reference, No_Location);
                           Set_Identifier (E, Make_Identifier
                     ...
                     C2 := Add_New_Connection
                       (No_Location,
                        Make_Identifier
                        (No_Location, Cnx_Number, Cnx_Number, No_node),
                        New_Process,
         ...
         if Copy then
            declare
               S2 : constant Node_Id :=  New_Node
                 (K_Subcomponent, No_Location);
            begin
   ...
   is
      ...
      pragma Assert (Kind (Thread) = K_Component_Implementation and then
                     Kind (Call_Seq) = K_Subprogram_Call_Sequence);
      N : Node_Id;
   begin
      ...
      if not Is_Empty (Connections (Thread)) then
         ...
         while Present (N) loop
            ...
            begin
               ...
               if Kind (Source_Port) = K_Port_Spec and then
                 Is_In_Call_Sequence
                 (Name (Item (First_Node (Path (Destination (N))))), Call_Seq)
               then
      ...
      begin
         ...
         N := AIN.First_Node (AIN.Features
                              (AIN.First_Node
                               (ATN.Instances (Thread))));
         while Present (N) loop
            if AIN.Kind (N) = AIN.K_Port_Spec_Instance then
               declare
                  Entrypoint : constant Name_Id := Get_String_Name
                    ("compute_entrypoint_call_sequence");
                  R          : Node_Id;
               begin
                  if Is_Defined_Reference_Property (N, Entrypoint) then
                     ...
                     if AIN.Name (AIN.Identifier (R)) =
                       ATN.Name (ATN.Identifier (Call_Seq)) then
   ...
   is
      ...
      function Copy_Subprogram_Call
        (Call, Owner_Sequence : Node_Id; Num : Int)
        return Node_Id;
      ...
      procedure Add_Entrypoint (New_Call_Seq, New_Thread : Node_Id;
      ...
      function Search_Parameter
        (Subprogram : Node_Id; Key : Name_Id)
        return Node_Id;
      ...
      begin
         Set_Property_Scope
           (Node, New_Node (K_Scope_Definition, No_Location));
      ...
      is
         ...
         Decl : constant Node_Id := Corresponding_Entity
           (Component_Type_Identifier (Subprogram));
         N    : Node_Id;
      begin
      ...
      is
         ...
         Wrapper          : constant Node_Id := Declare_Wrapper_Subprogram
            (New_Thread);
         Sp               : Node_Id;
         ...
         begin
            ...
            while Present (Cnx) loop
               declare
                  ...
                  Src      : constant Node_Id := First_Node
                    (Path (Source (Cnx)));
                  Dst      : constant Node_Id := First_Node
                  ...
                  Param, C : Node_Id;
                  Cnx_Number : constant Name_Id := Get_String_Name
                    ("cnx_" & Image (Cnx_Num));
                  Ignore   : Boolean := False;
               begin
                  if (Kind (Corresponding_Entity (Item (Src))) /= K_Port_Spec
                      and then Kind (Corresponding_Entity (Item (Src))) /=
                      K_Parameter
                      and then No
                      (Search_Subprogram_By_Name
                       (Name (Item (Src)), Local_Subprogram_Mapping)))
                    or else
                    (Kind (Corresponding_Entity (Item (Dst))) /= K_Port_Spec
                     and then Kind (Corresponding_Entity (Item (Dst))) /=
                     K_Parameter
                     and then No
                     (Search_Subprogram_By_Name
                      (Name (Item (Dst)), Local_Subprogram_Mapping)))
                  then
                     Ignore := True;
                  end if;

                  if not Ignore and then
                    Kind (Corresponding_Entity (Item (Src))) = K_Port_Spec then

                     Src_Feat := Find_New_Feature_Node
                       (Name (Item (Src)), Parent_Thread, Feature_Mapping);
                     ...
                     if No (Param) then
                        ...
                        if Is_Event (Src_Feat) and then
                          ...
                          K_Port_Spec then
                           ...
                           Param := Add_New_Port_Spec
                             (No_Location,
                              Make_Identifier
                              (No_Location, Src_Name, Src_Name, No_node),
                              Corresponding_entity
                        ...
                        Set_Entity_Ref
                          (Param, Entity_Ref
                           (Corresponding_Entity (Item (Src))));
                     end if;
                     Cnx_Num := Cnx_Num + 1;
                     declare
                        use Ocarina.Builder.AADL.Components.Connections;

                        Cnx_Num_Name : constant Name_Id := Get_String_Name
                          ("cnx_" & Image (Cnx_Num));
                     ...
                     begin
                        Create_Connection_End_Path
                          (Dst_Path_2, Wrapper_Name, Src_Name);
                        ...
                        Create_Connection
                          (Src_Path_2, Dst_Path_2, No_Node,
                           New_Thread, CT, Cnx_Num_Name);
                     end;

                  elsif not Ignore and then
                    Kind (Corresponding_Entity (Item (Src))) = K_Parameter then
                     ...
                     if No (Sp) then
                        ...
                        Src_Name := Name (Sp);
                        Create_Connection_End_Path
                          (Src_Path, Src_Name, Name (Item (Next_Node (Src))));
                     end if;
                  end if;

                  if not Ignore and then
                    Kind (Corresponding_Entity (Item (Dst))) = K_Port_Spec then

                     Dst_Feat := Find_New_Feature_Node
                       (Name (Item (Dst)), Parent_Thread, Feature_Mapping);
                     ...
                     if No (Param) then
                        if Is_Event (Dst_Feat) then
                           ...
                           Param := Add_New_Port_Spec
                             (No_Location,
                              Make_Identifier
                              (No_Location, Dst_Name, Dst_Name, No_node),
                              Corresponding_entity
                              (Component_Type_Identifier (Wrapper)),
                              Is_In (Dst_Feat),
                              Is_Out (Dst_Feat),
                     ...
                     begin
                        Create_Connection_End_Path
                          (Src_Path_2, Wrapper_Name, Dst_Name);
                    ...
                    Kind (Corresponding_Entity (Item (Dst))) = K_Parameter then
                  ...
                  if not Ignore then
                     if Category (Cnx) =
                       Connection_Type'Pos (CT_Parameter) then
                        Create_Connection
                          (Src_Path, Dst_Path, No_Node,
                           Wrapper, CT_Parameter, Cnx_Number);
                     else
                        Create_Connection
                          (Src_Path, Dst_Path, No_Node,
                           Wrapper, CT_Port_Connection, Cnx_Number);
                     end if;
                     Cnx_Num := Cnx_Num + 1;
                  end if;
               end;
      ...
      begin
         ...
         if Length (Subprogram_Calls (Call_Seq)) = 1 then
            Sp := Entity (Entity_Ref
                          (First_Node
                           (Subprogram_Calls (Call_Seq))));
            if Kind (Sp) = K_Component_Implementation and then
              Calls (Sp) /= No_List and then not Is_Empty (Calls (Sp)) then
         ...
         Wrapper_Call_Seq := Add_New_Subprogram_Call_Sequence
           (No_Location,
            ...
            Wrapper);
         Add_Call_Sequence
           (Target_CS, Wrapper_Call_Seq, Local_Subprogram_Mapping);

         --  3/ We create the top-level call sequence

         Append_Subprogram_Call
           (New_Call_Seq, Wrapper, "main_" & Image (Wrapper_Num));
         Wrapper_Num := Wrapper_Num + 1;
         Wrapper_Name := Name (Identifier
                               (First_Node
                                (Subprogram_Calls ((New_Call_Seq)))));
         ...
         if Is_Wrapper then
            ...
            if Is_Defined_Property (Old_Thread, WCET) then
               ...
               Display_Located_Error
                 (Loc (Old_Thread), "WCET must be declared",
                  Fatal => True);
            end if;
         end if;
      ...
      is
         ...
         Sp, Sp_Cpy : Node_Id;
         Last       : constant Node_Id := Last_Node
           (Subprogram_Calls (Call_Seq_Src));
      begin
         ...
         while Present (Sp) loop
            if Sp /= Last or else
              Sp = First_Node (Subprogram_Calls (Call_Seq_Src)) or else
              Name (Identifier (Sp)) /= Scheduler_Call then
      ...
      is
         ...
         pragma Assert (Kind (New_Thread) = K_Component_Implementation);
         pragma Assert (Kind (Old_Thread) = K_Component_Implementation);
         pragma Assert (Mode_Binding = No_Node or else
                          Kind (Mode_Binding) = K_In_Modes);
         ...
         L2            : List_Id := No_List;
         CS_Name       : constant Name_Id := Name
           (Identifier (New_Call_Seq));
      begin
         ...
         Id := Make_Identifier
           (No_Location, Compute_Entrypoint, Compute_Entrypoint, No_Node);
         S2 := New_Node (K_Entity_Reference, No_Location);
         ...
         if Present (Port) then
            ...
            New_Feature := Find_New_Feature_Node
              (Name (Identifier (Port)), Old_Thread, Port_Mapping);
            ...
            Append_Node_To_List
              (Make_Identifier
               (No_Location, New_Port_Name, New_Port_Name, No_Node), L2);
         ...
         S := Make_Identifier
           (No_Location, CS_Name, CS_Name, No_Node);
      ...
      is
         ...
         Call_Seq_Name : constant Name_Id := Get_String_Name
           ("cs_" & Image (Call_Seq_Num));
      begin
      ...
      function Create_Stop_Mode (New_Thread_Impl : Node_Id) return Node_Id
      is
         New_Mode     : constant Node_Id := New_Node
           (K_Mode, Loc (New_Thread_Impl));
      ...
      begin
         ...
         Set_Identifier
           (New_Mode, Make_Identifier
            (No_Location, Stop_Mode_Name, Stop_Mode_Name, New_Mode));
         Append_Node_To_List
           (New_Mode, Modes (New_Thread_Impl));
         ...
         declare
            ...
            Mode_Ref     : constant Node_Id := New_Node
              (K_Entity_Reference, Loc (New_Thread_Impl));
         ...
         begin
            ...
            Set_Identifier
              (Mode_Ref, Make_Identifier
               (Loc (New_Call_Seq), Stop_Mode_Name, Stop_Mode_Name, No_Node));
            ...
            if not Add_Subprogram_Call_Sequence
              (New_Thread_Impl, New_Call_Seq) then
               raise Program_Error;
            end if;
            ...
            begin
               Id := Make_Identifier
                 (No_Location, Compute_Entrypoint,
                  Compute_Entrypoint, No_Node);
               S2 := New_Node (K_Entity_Reference, No_Location);
               Set_Identifier (S2, Id);
               S := Make_Identifier
                 (No_Location, CS_Name, CS_Name, No_Node);
               ...
               Mode_List := New_Node (K_In_Modes, Loc (New_Call_Seq));
               Set_Modes (Mode_List,
                          New_List (K_List_Id, Loc (New_Call_Seq)));
               Set_Identifier
                 (Mode_Ref, Make_Identifier
                  (Loc (New_Call_Seq), Stop_Mode_Name,
                   Stop_Mode_Name, No_Node));
               Append_Node_To_List (Mode_Ref, Modes (Mode_List));
               Set_Full_Identifier (S2, Id);
               Id := Add_New_Property_Association
                 (Loc            => No_Location,
      ...
      is
         ...
         is
            ...
            Thread_Decl  : constant Node_Id := Corresponding_Entity
              (Component_Type_Identifier (Thread));
            Priority     : Int;
            Call_Seq     : Node_Id;
            P            : Node_Priority;
            Register     : Boolean;
         begin
            ...
            while Present (Call_Seq) loop
               ...
               if Present (In_Modes (Call_Seq)) and then
                 not Is_Empty (Modes (In_Modes (Call_Seq))) and then
                 Length (Modes (In_Modes (Call_Seq))) = 1 and then
               ...
               if Register then
                  ...
                  if Priority = 0 then
                     Priority := Int (Get_Integer_Property
                                      (Thread, Cheddar_Priority));
                     if Priority = 0 then
                        Priority := Int (Get_Integer_Property
                                         (Thread_Decl, Cheddar_Priority));
                     end if;
                  end if;

                  if Priority = 0 then
                     DE (Image (Loc (Thread)) & " : priority must be "
      ...
      is
         ...
         function Declare_Object_With_Priority
           (New_Thread : Node_Id; P : Int) return Node_Id;
         ...
         is
            ...
            Dummy_Type : Node_Id;
            Impl       : Node_Id;
            Data_Name  : constant Name_Id := Get_String_Name
              ("dummy_internal_type");
            Decl_Id    : constant Node_Id := Make_Identifier
              (No_Location, Data_Name, Data_Name, No_Node);
            Impl_Name  : constant Name_Id := Get_String_Name
              ("dummy_internal_type.i");
            Impl_Id    : constant Node_Id := Make_Identifier
              (No_Location, Impl_Name, Impl_Name, No_Node);
            Struct     : constant Name_Id := Get_String_Name ("integer");
            S, Id, S2  : Node_Id;
         begin

            Dummy_Type := Find_Component_Classifier
              (AADL_Root, Identifier (Namespace (New_Thread)), Decl_Id);
            if No (Dummy_Type) then
               Dummy_Type := Add_New_Component_Type
                 (No_Location,
                  Decl_Id,
                  Namespace (New_Thread),
                  CC_Data,
               ...
               S := New_Node (K_Property_Term, No_Location);
               Set_Identifier
                 (S, Make_Identifier (No_Location, Struct, Struct,  No_Node));
               Id := Make_Identifier
                 (No_Location, Data_Representation,
                  Data_Representation, No_Node);
               S2 := New_Node (K_Entity_Reference, No_Location);
               Set_Identifier
                 (S2, Make_Identifier (No_Location, Raw_Data_Representation,
                                       Raw_Data_Representation, No_Node));
               Set_Namespace_Identifier
                 (S2, Make_Identifier (No_Location, Data_Model_Pkg,
                                       Data_Model_Pkg, No_Node));
               Set_Full_Identifier (S2, Id);
            ...
            Impl := Find_Component_Classifier
              (AADL_Root, Identifier (Namespace (New_Thread)), Impl_Id);
            if No (Impl) then
               Impl := Add_New_Component_Implementation
                 (No_Location,
                  Impl_Id,
         ...
         is
            ...
            Concurrency : constant Name_Id := Get_String_Name
              ("concurrency_control_protocol");
            PCP         : constant Name_Id := Get_String_Name
              ("priority_ceiling");
            Data_Representation : constant Name_Id := Get_String_Name
              ("data_model::data_representation");
            Struct     : constant Name_Id := Get_String_Name ("struct");

            Decl_Name  : constant Name_Id := Get_String_Name
              ("priority_switcher_" & Image (P));
            Decl_Id    : constant Node_Id := Make_Identifier
              (No_Location, Decl_Name, Decl_Name, No_Node);
            Impl_Name  : constant Name_Id := Get_String_Name
              (Get_Name_String (Decl_Name) & ".i");
            Impl_Id    : constant Node_Id := Make_Identifier
              (No_Location, Impl_Name, Impl_Name, No_Node);
            Obj_Name   : constant Name_Id := Get_String_Name
              (Get_Name_String (Decl_Name) & "_obj");
            L          : constant Node_Id := New_Node (K_Literal, No_Location);
         ...
         begin
            ...
            Old_Comp_Decl := Find_Component_Classifier
              (AADL_Root, Identifier (Pkg), Decl_Id);
            if Present (Old_Comp_Decl) then
               Remove_From_Scope
                 (Identifier (Old_Comp_Decl), Entity_Scope (Pkg));
               Remove_Node_From_List (Old_Comp_Decl, Declarations (Pkg));
            end if;
            Old_Comp_Decl := Find_Component_Classifier
              (AADL_Root, Identifier (Pkg), Impl_Id);
            if Present (Old_Comp_Decl) then
               Remove_From_Scope
                 (Identifier (Old_Comp_Decl), Entity_Scope (Pkg));
               Remove_Node_From_List (Old_Comp_Decl, Declarations (Pkg));
            ...
            Comp_Decl := Add_New_Component_Type
              (No_Location,
               Decl_Id,
               Namespace (New_Thread),
               CC_Data,
            ...
            S := New_Node (K_Property_Term, No_Location);
            Set_Identifier
              (S, Make_Identifier (No_Location, PCP, PCP, No_Node));
            Id := Make_Identifier
              (No_Location, Concurrency, Concurrency, No_Node);
            S2 := New_Node (K_Entity_Reference, No_Location);
            Set_Identifier (S2, Id);
            Set_Full_Identifier (S2, Id);
            Id := Add_New_Property_Association
            ...
            Set_Value (L, New_Integer_Value
                       (Unsigned_Long_Long (P)));
            Id := Make_Identifier
              (No_Location, Raw_Priority, Raw_Priority, No_Node);
            S2 := New_Node (K_Entity_Reference, No_Location);
            Set_Identifier (S2, Id);
            Set_Full_Identifier (S2, Id);
            Id := Add_New_Property_Association
            ...
            Id := Make_Identifier
              (No_Location, Priority_Shifter, Priority_Shifter, No_Node);
            S2 := New_Node (K_Entity_Reference, No_Location);
            Set_Identifier
              (S2, Make_Identifier
               (No_Location, Raw_Priority_Shifter,
                Raw_Priority_Shifter, No_Node));
            Set_Namespace_Identifier
              (S2, Make_Identifier
               (No_Location, Transfo_Pkg, Transfo_Pkg, No_Node));
            Set_Full_Identifier (S2, Id);
            ...
            Old_Comp_Impl := Find_Component_Classifier
              (AADL_Root, Identifier (Pkg), Impl_Id);
            if Present (Old_Comp_Impl) then
               Remove_Node_From_List (Old_Comp_Impl, Declarations (Pkg));
            end if;

            Comp_Impl := Add_New_Component_Implementation
              (No_Location,
               Impl_Id,
               Namespace (New_Thread),
               CC_Data,
               Is_Private => False);
            Set_Component_Type_Identifier
              (Comp_Impl, Identifier (Comp_Decl));
            ...
            Set_Identifier
              (S2, Make_Identifier
               (No_Location, Raw_Data_Representation,
                Raw_Data_Representation, No_Node));
            Set_Namespace_Identifier
              (S2, Make_Identifier
               (No_Location, Data_Model_Pkg, Data_Model_Pkg, No_Node));
            Set_Full_Identifier (S2, Id);
            ...
            Object := Add_New_Subcomponent
              (No_Location,
               Make_Identifier (No_Location, Field_Name, Field_Name, No_Node),
               Comp_Impl, CC_Data);
      ...
      is
         ...
         is
            ...
            Obj_Decl : constant Node_Id := Corresponding_Entity
              (Component_Type_Identifier (Obj_Impl));
            ...
            Mtd   : constant Name_Id := Get_String_Name
              ("method_" & Image (Method_Num));
            E     : Node_Id;
            Param : Node_Id;
            Cnx   : Node_Id;
         ...
         begin
            ...
            Param := Add_New_Subcomponent_Access
              (No_Location,
            ...
            E := New_Node (K_Entity_Reference, No_Location);
            Set_Identifier (E, Make_Identifier
                            (No_Location, Obj_Name, Obj_Name, No_Node));
            ...
            E := New_Node (K_Entity_Reference, No_Location);
            Set_Identifier (E, Make_Identifier
                            (No_Location, Spg_N, Spg_N, No_Node));
            Set_Entity_Ref (Param, E);
            Set_Entity (E, Spg);
            ...
            begin
               ...
               Set_Identifier (E, Make_Identifier
                               (No_Location, New_Call, New_Call, No_Node));
               New_Path := New_List (K_List_Id, No_Location);
               Create_Connection_End_Path
                 (New_Path, Name (Identifier (Obj_Decl)), Mtd);
               Set_Path (E, New_Path);
               Set_Entity (E, Param);
               ...
               New_Path := New_List (K_List_Id, No_Location);
               Id := Make_Identifier
                 (No_Location, Call_Name, Call_Name, No_Node);
               Set_Identifier (Dst, Id);
               Create_Connection_End_Path
                 (New_Path, Name (Identifier (Wrapper_Call)), This);
               Set_Path (Dst, New_Path);

               Cnx := Add_New_Connection
                 (No_Location,
                  Make_Identifier
                  (No_Location, Cnx_Number, Cnx_Number, No_node),
                  Thread,
      ...
      begin
         ...
         while Present (Call_Seq) loop
            ...
            T_Call_Seq := Call_Seq;
            if Register and then
              Kind (Entity
                    (Entity_Ref
                     (First_Node
                      (ATN.Subprogram_Calls (Call_Seq))))) =
              K_Subcomponent_Access then
               declare
                  Obj : constant Node_Id := Container_Component
                    (Entity
                     (Entity_Ref
                      (First_Node
                       (ATN.Subprogram_Calls (Call_Seq)))));
               begin
                  ...
                  if Kind (Obj) = K_Component_Type and then
                    Is_Defined_Property (Obj, Priority_Shifter)
                  then
                     declare
                        N    : constant Node_Id := Create_Call_Sequence;
                        SPC  : constant Node_Id := New_Node
                          (K_Subprogram_Call, No_Location);
                        Ref  : constant Node_Id := New_Node
                          (K_Entity_Reference, No_Location);
                        Call : constant Name_Id := Get_String_Name ("main");
                        Base_Spg : constant Node_Id := Entity
                          (Entity_Ref
                           (First_Node
                            (ATN.Subprogram_Calls (Call_Seq))));
                     begin

                        Set_Identifier (SPC, Make_Identifier
                                        (No_Location, Call, Call, No_Node));
                        Set_Identifier
                          (Ref, Make_Identifier
                           (No_Location, Name (Identifier (Base_Spg)),
                            Name (Identifier (Base_Spg)), No_Node));
                        Set_Entity_Ref (SPC, Ref);
                        Set_Entity (Ref, Entity (Entity_Ref (Base_Spg)));

                        ATN.Set_Subprogram_Calls
                          (N, New_List (K_List_Id, No_Location));
            ...
            if Register then
               ...
               Copy_Call_Sequence
                 (T_Call_Seq, Thread_Impl, New_Thread_Impl,
                  Thread_Mapping, New_Call_Seq, Wrapper_Wcet);
               if not Add_Subprogram_Call_Sequence
                 (New_Thread_Impl, New_Call_Seq) then
                  raise Program_Error;
               end if;
               ...
               if not Is_Call_Sequence_Sporadic (T_Call_Seq, Thread_Impl) then
                  ...
                  if Build_Modes then
                     ...
                     declare
                        New_Mode  : constant Node_Id := New_Node
                       (K_Mode, No_Location);
                        Mode_Name : constant Name_Id := Get_String_Name
                          ("mode_" & Image (Periodic_Counter));
                        Mode_Ref  : constant Node_Id := New_Node
                          (K_Entity_Reference, No_Location);
                     begin
                        --  Create a new mode
                        ...
                        Set_Modes (Mode_List, New_List
                                   (K_List_Id, No_Location));
                        Set_Identifier
                          (Mode_Ref, Make_Identifier
                           (No_Location, Mode_Name, Mode_Name, No_Node));
                        Set_Entity (Mode_Ref, New_Mode);
                        Append_Node_To_List (Mode_Ref, Modes (Mode_List));
                        Set_In_Modes (New_Call_Seq, Mode_List);
                        ...
                        if Is_Empty (Modes (New_Thread_Impl)) then
                           Set_Modes (New_Thread_Impl,
                                      New_List (K_List_Id, Loc (Thread_Impl)));
                        end if;
                        Set_Container_Component (New_Mode, New_Thread_Impl);
                        Set_Identifier
                          (New_Mode, Make_Identifier
                           (Loc (T_Call_Seq), Mode_Name, Mode_Name, New_Mode));
                        Append_Node_To_List
                          (New_Mode, Modes (New_Thread_Impl));
                        Add_Call_To_Scheduler (New_Call_Seq);
                     end;
                  end if;
                  Add_Entrypoint (New_Call_Seq, New_Thread_Impl,
                                  Call_Seq, Thread_Impl, Mode_List,
                                  Thread_Mapping);
               else
                  Is_Sporadic := True;
                  Add_Entrypoint (New_Call_Seq, New_Thread_Impl,
                                  Call_Seq, Thread_Impl, No_Node,
                                  Thread_Mapping);
               end if;
               ...
               Obj := Find_First_Object_With_Same_Priority
                 (Priorities, Call_Seq);
               ...
               if Present (Obj) then
                  ...
                  Add_Subprogram_To_Data
                    (Obj, First_Node (ATN.Subprogram_Calls (New_Call_Seq)),
                     New_Thread_Impl);
               end if;
            end if;
   ...
   begin
      ...
      Register_Priorities
        (Th1_Impl, Th2_Impl, Priorities, Minimum_Priority, Maximum_Priority);
--        Build_Protected_Objects
--          (New_Thread_Impl, Priorities, Minimum_Priority);
      ...
      Copy_All_Call_Sequences
        (Th1_Impl, New_Thread_Impl, Periodic_Counter, Th1_Mapping,
         Priorities, Build_Modes, Is_Src_Sporadic, Is_Src_Periodic);

      Copy_All_Call_Sequences
        (Th2_Impl, New_Thread_Impl, Periodic_Counter, Th2_Mapping,
         Priorities, Build_Modes, Is_Dst_Sporadic, Is_Dst_Periodic);
      ...
      Ocarina.Transfo.Fusions.Scheduler.Initialize
        (New_Thread_Impl, Th1_Impl, Th2_Impl);
      ...
      if Build_Modes then
         declare
            ...
            S : Node_Id;
            I : Node_Id;
            Package_Name : constant Name_Id := AIN.Name
              (AIN.Identifier (AIN.Namespace (Th_Inst_1)));
      ...
      Manage_Component_Properties
        (Th1_Impl, Th2_Impl, New_Thread_Impl, Success,
         Sporadic, Periodic);
   ...
   is
      ...
      pragma Assert (Kind (S) = K_Component_Implementation or else
                     Kind (S) = K_Component_Type);
   ...
   begin
      ...
      if Get_Category_Of_Component (S) = CC_Subprogram then
         ...
         Id := Make_Identifier
           (No_Location, CS_Period, CS_Period, No_Node);
   ...
   begin
      ...
      if S /= No_Node then
         ...
         Decl := ATN.Corresponding_Entity
           (ATN.Component_Type_Identifier (E));
         S := Ocarina.Analyzer.AADL.Queries.Get_Property_Association
           (Decl, WCET);
         if S /= No_Node then
      ...
      ATN.Set_Value (V2, New_Integer_Value
                     (Unsigned_Long_Long (Max_Duration)));
      ...
      S2 := New_Node (K_Entity_Reference, No_Location);
      Set_Identifier (S2, Make_Identifier
                      (No_Location, WCET, WCET, No_Node));
      Set_Full_Identifier (S2, Id);
   ...
   begin
      S2 := New_Node (K_Entity_Reference, No_Location);
      Id := Make_Identifier
        (No_Location, Deadline, Deadline, No_Node);
      Set_Identifier
        (S2, Make_Identifier
         (No_Location, Deadline, Deadline, No_Node));
   ...
   begin
      ...
      case Get_Category_Of_Component (S) is
         ...
         when CC_Thread =>
            Id := Make_Identifier
              (No_Location, Cheddar_Priority, Cheddar_Priority, No_Node);
            Set_Identifier
              (S2, Make_Identifier
               (No_Location, Raw_Fixed_Priority, Raw_Fixed_Priority, No_Node));
            Set_Namespace_Identifier
              (S2, Make_Identifier
               (No_Location, Cheddar_Pkg, Cheddar_Pkg, No_Node));

         when CC_Subprogram =>
            Id := Make_Identifier
              (No_Location, Transfo_Priority, Transfo_Priority, No_Node);
            Set_Identifier
              (S2, Make_Identifier
               (No_Location, Raw_Priority, Raw_Priority, No_Node));
            Set_Namespace_Identifier
   ...
   is
      ...
      Decl : Node_Id;
      Impl : Node_Id;
      Decl_Id_Name : constant Name_Id := Build_Unique_Name
        (Declarations (Namespace (Owner_Thread)), Wrapper_Prefix);
      Impl_Id_Name : Name_Id;
   begin
      Impl_Id_Name := Get_String_Name
        (Get_Name_String (Decl_Id_Name) & ".i");
   ...
   is
      ...
      pragma Assert ((Kind (Old_Thread_1) = K_Component_Implementation or else
                      Kind (Old_Thread_1) = K_Component_Type) and then
                     (Kind (Old_Thread_2) = K_Component_Implementation or else
                      Kind (Old_Thread_2) = K_Component_Type) and then
                     (Kind (New_Thread) = K_Component_Implementation or else
                      Kind (New_Thread) = K_Component_Type));
      ...
      is
         ...
         pragma Assert (Kind (Old_Thread) =
                        K_Component_Implementation or else
                        Kind (Old_Thread) = K_Component_Type);
         pragma Assert (Kind (New_Thread) =
                        K_Component_Implementation or else
                        Kind (New_Thread) = K_Component_Type);
         ...
         while Present (P) loop
            ...
            if P_Name = Dispatch_Protocol then
               ...
               begin
                  if Present
                    (Single_Value
                     (Property_Association_Value (P))) then
                     Property_Val := Single_Value
                       (Property_Association_Value (P));
                  else
                     --  FIXME
                     --  Very dirty !
                     ...
                     Property_Val := Node_Id
                       (Multi_Value (Property_Association_Value (P)));
                  end if;

                  Prop := Add_New_Property_Association
                    (Loc => No_Location,
   ...
   begin
      ...
      if Is_Periodic or else Is_Sporadic then
         ...
         declare
            S, S2, Id     : Node_Id;
            Property_Name : constant Name_Id := Get_String_Name
              ("dispatch_protocol");
         ...
         begin
            ...
            Set_Identifier
              (S, Make_Identifier
               (No_Location, Chosen, Chosen, No_Node));
            Id := Make_Identifier
              (No_Location, Property_Name, Property_Name, No_Node);
            S2 := New_Node (K_Entity_Reference, No_Location);
            Set_Identifier (S2, Id);
            Set_Full_Identifier (S2, Id);
            Id := Add_New_Property_Association
         ...
         begin
            ...
            if Gcd = 0 then
               if Is_Defined_Property (Old_Thread_1, Period) then
                  ...
                  --  Should be periodic !
                  Display_Located_Error
                    (Loc (Old_Thread_1), "should be periodic",
                     Fatal => True);
               end if;
            end if;
         ...
         declare
            use Ocarina.AADL_Values;
               ...
               Id, S2 : Node_Id;
         begin
            Id := Make_Identifier (No_Location, Transfo_Occurred,
                                   Transfo_Occurred, No_Node);
            S2 := New_Node (K_Entity_Reference, No_Location);
            Set_Identifier (S2, Make_Identifier
                            (No_Location, Raw_Occurred,
                             Raw_Occurred, No_Node));
            Set_Namespace_Identifier (S2, Make_Identifier
                                      (No_Location, Transfo_Pkg,
                                       Transfo_Pkg, No_Node));
   ...
   is
      ...
      pragma Assert (ATN.Kind (Old_Proc_Subcomponent) =
                     ATN.K_Subcomponent);
   ...
   begin
      ...
      if Present (Parent (System)) and then
        Kind (Parent (System)) = K_Entity_Reference then
      ...
      ATN.Set_Is_Implicit_Inverse
        (S, ATN.Is_Implicit_Inverse (Old_Proc_Subcomponent));
      ATN.Set_Inversed_Entity
        (S, ATN.Inversed_Entity (Old_Proc_Subcomponent));
      ...
      ATN.Set_Namespace_Path
        (E, Namespace_Path (ATN.Entity_Ref (Old_Proc_Subcomponent)));
      L := New_List (K_List_Id, No_Location);
      Append_Node_To_List
        (Make_Identifier
         (No_Location,
          ...
          No_Node), L);
      ATN.Set_Path (E, L);
      ATN.Set_Identifier (E, Make_Identifier
                      (No_Location,
                       ATN.Name (ATN.Identifier (New_Proc)),
                       ATN.Display_Name (ATN.Identifier (New_Proc)),
                       E));
      ...
      ATN.Set_Properties
        (S, ATN.Properties (Old_Proc_Subcomponent));

      Remove_From_Scope
        (Identifier (Old_Proc_Subcomponent), Entity_Scope (R));
   ...
   is
      ...
      pragma Assert (Kind (Old_Thread_1) = K_Component_Implementation and then
   ...
   begin
      ...
      while Present (N) loop
         ...
         begin
            ...
            if (Kind (Corresponding_Entity (Item (S))) = K_Port_Spec
                or else (Present (Next_Node (S)) and then
                         Kind (Corresponding_Entity (Item (Next_Node (S))))
                         = K_Port_Spec)) and then
              (Kind (Corresponding_Entity (Item (S2))) = K_Port_Spec
               or else (Present (Next_Node (S2)) and then
                        Kind (Corresponding_Entity (Item (Next_Node (S2))))
                        = K_Port_Spec)) then

               Is_Src_Th1 := (To_Lower
                              (Get_Name_String (Name (Item (S)))) =
                              Th1_Inst.all);
               Is_Src_Th2 := (To_Lower
                              (Get_Name_String (Name (Item (S)))) =
                              Th2_Inst.all);
               Is_Dst_Th1 := (To_Lower
                              (Get_Name_String (Name (Item (S2)))) =
                              Th1_Inst.all);
               Is_Dst_Th2 := (To_Lower
                              (Get_Name_String (Name (Item (S2)))) =
                              Th2_Inst.all);
               ...
               if Is_Src_Th1 then
                  T := Find_New_Feature_Node
                    (Name (Item (Next_Node (S))), Old_Thread_1, Th1_Mapping);
                  ...
                  Set_Source
                    (Cnx, New_Node (K_Entity_Reference, No_Location));
               ...
               if Is_Dst_Th1 then
                  T := Find_New_Feature_Node
                    (Name (Item (Next_Node (S2))), Old_Thread_1, Th1_Mapping);
                  Increment_Related_Counter (T);
                  if No (T) then
                     DE ("feature " &
                  ...
                  T := Find_New_Feature_Node
                    (Name (Item (Next_Node (S2))), Old_Thread_2, Th2_Mapping);
                  Increment_Related_Counter (T);
                  F1 := Name (Identifier (T));
                  if F1 = No_Name then
                     DE ("Could not find feature " &
               ...
               Id := Make_Identifier
                 (No_Location, F1, F1, Destination (Cnx));
   ...
   is
      ...
      pragma Assert (No (Old_Cnx) or else
                     Kind (Old_cnx) = K_Connection);
   ...
   is
      pragma Assert (Kind (Old_Decl) = K_Component_Type or else
                     Kind (Old_Decl) = K_Component_Implementation);
   begin
   ...
   begin
      ...
      while Present (N) and then Same_Loc loop
         Same_Loc := No (Prev_Inst) or else
           AIN.Loc (N) = AIN.Loc (Prev_Inst);
         Prev_Inst := N;
         N := AIN.Next_Node (N);
      end loop;
      ...
      if Same_Loc then
         Remove_Node_From_List
           (Old_Impl, Declarations (Namespace));
      end if;
   ...
   begin
      ...
      if Same_Loc then
         Remove_Node_From_List
           (Old_Decl, Declarations (Namespace));
      end if;
   ...
   is
      ...
      pragma Assert (Kind (Thread_Component) =
                     K_Component_Type);
   ...
   procedure Copy_All_Subcomponents
     (Features_List : List_Id;
      Thread_Impl, Owner_Thread : Node_Id)
   is
      pragma Assert (Kind (Thread_Impl) =
   ...
   procedure Copy_Feature_Core
     (Feature, New_Feature, Owner_Component : Node_Id; Suffix : String := "")
   is
      Id : Node_Id;
      Property_Scop : constant Node_Id :=
        New_Node (K_Scope_Definition, No_Location);
   begin
      ...
      if Suffix = "" then
         if No (Identifier (Feature)) then
            ...
            Id := Make_Identifier (No_Location,
         ...
         begin
            if No (Identifier (Feature)) then
               ...
               Concat := new
                 String'(Get_Name_String (Name (Identifier (Feature))) &
                         Suffix);
               Str_Name := Get_String_Name (Concat.all);
               Id := Make_Identifier (No_Location,
                                      Str_Name,
   ...
   is
      ...
      It          : Natural := First;
      Parent_Type : constant Node_Id := Corresponding_Entity
        (Component_Type_Identifier (Parent));
   begin
      while It <= Last (Mapping) loop
         if To_Lower (Name (Identifier (Mapping.Table (It).Old_Node))) =
           To_Lower (Old_Feature_Name) then
            declare
   ...
   is
      pragma Assert (Kind (Component) = K_Component_Implementation or else
   ...
   begin
      if Kind (Component) = K_Component_Implementation then
         ...
         if OAQ.Is_Defined_Integer_Property (Component, Property) then
            ...
            Display_Located_Error
              (Loc (Component), "integer property " &
               Get_Name_String (Property) & " no found", Fatal => True);
         end if;
      elsif Kind (Component) = K_Component_Type then
         if OAQ.Is_Defined_Integer_Property (Component, Property) then
            return OAQ.Get_Integer_Property (Component, Property);
   ...
   begin
      if Kind (Component) = K_Component_Implementation then
         ...
         if OAQ.Is_Defined_Boolean_Property (Component, Property) then
            ...
            Display_Located_Error
              (Loc (Component), "boolean property " &
               Get_Name_String (Property) & " no found", Fatal => True);
         end if;
      elsif Kind (Component) = K_Component_Type then
         if OAQ.Is_Defined_Boolean_Property (Component, Property) then
