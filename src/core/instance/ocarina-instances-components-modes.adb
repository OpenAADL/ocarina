...
package body Ocarina.Instances.Components.Modes is
      ...
      begin
         if No (Path_Element) then
            ...
            raise Program_Error with
              "FATAL: A feature path that does not end with a feature";
         end if;

         pragma Assert (AIN.Kind (Component) = K_Component_Instance and then
                        ATN.Kind (Path_Element) = K_Node_Container and then
                        ATN.Kind (ATN.Item (Path_Element)) = K_Identifier);
         ...
         case ATN.Kind (ATN.Corresponding_Entity (ATN.Item (Path_Element))) is
            ...
            when K_Subprogram_Call =>
               ...
               declare
                  Call_Seq : Node_Id :=
                    ATN.First_Node (ATN.Calls (Component));
               begin
   ...
   begin
      pragma Assert (Kind (Instance_Root) = K_Architecture_Instance   and then
                     Kind (Component_Instance) = K_Component_Instance and then
                     Kind (Mode) = K_Mode);

      New_Instance := New_Node (K_Mode_Instance, ATN.Loc (Mode));

      AIN.Set_Corresponding_Declaration (New_Instance, Mode);
      AIN.Set_Identifier (New_Instance, AIE.Duplicate_Identifier
                        (ATN.Identifier (Mode)));
   ...
   begin
      pragma Assert (Kind (Instance_Root) = K_Architecture_Instance   and then
                     Kind (Component_Instance) = K_Component_Instance and then
                     Kind (Mode_Transition) = K_Mode_Transition);

      New_Instance := New_Node (K_Mode_Transition_Instance,
                                ATN.Loc (Mode_Transition));
      ...
      while Present (N) loop
         M := Get_First_Homonym_Instance
           (AIN.Modes (Component_Instance), ATN.Name (N));
         ...
         AINU.Append_Node_To_List
           (Make_Node_Container (M), AIN.Source_Modes (New_Instance));
   ...
   begin
      pragma Assert
        (Kind (Component_Instance) = K_Component_Instance and then
         (Kind (Subclause_Instance) = K_Subcomponent_Instance or else
          Kind (Subclause_Instance) = K_Call_Sequence_Instance or else
          Kind (Subclause_Instance) = K_Connection_Instance or else
          Kind (Subclause_Instance) = K_Property_Association_Instance));

      if AIN.Kind (Subclause_Instance) = K_Property_Association_Instance then
         Subclause_In_Modes := AIN.In_Modes (Subclause_Instance);
      else
         Subclause_In_Modes := ATN.In_Modes
      ...
      then
         ...
         declare
            Mode_List : List_Id
              renames ATN.Modes (AIN.In_Modes (Subclause_Instance));
         begin
            while Present (MoT) loop
               case ATN.Kind (MoT) is
                       ...
                       (Make_Node_Container
                        (Get_First_Homonym
                         (ATN.Modes (Component_Instance),
                          ATN.Name (ATN.Identifier (First_Reference (MoT)))),
                         Get_First_Homonym
                         (ATN.Modes (Component_Instance),
                          ATN.Name (ATN.Identifier (Second_Reference (MoT))))),
                        Mode_List);
                  ...
                  when others =>
                     ...
                     raise Program_Error with
                       "A non-mode-non-transition in an in_mode list";
               end case;
