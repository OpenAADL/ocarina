...
package body Ocarina.Instances.Components.Subprogram_Calls is
   ...
   is
      ...
      New_Instance    : constant Node_Id := New_Node
        (K_Call_Sequence_Instance, ATN.Loc (Call_Sequence));
   ...
   begin
      AIN.Set_Identifier (New_Instance,
                          AIE.Duplicate_Identifier
                            (ATN.Identifier (Call_Sequence)));
      Set_Corresponding_Declaration (New_Instance, Call_Sequence);

      if ATN.Subprogram_Calls (Call_Sequence) /= No_List then
         AIN.Set_Subprogram_Calls (New_Instance,
                                   New_List (K_List_Id,
                                   ATN.Loc (Node_Id
                                   (ATN.Subprogram_Calls (Call_Sequence)))));
         List_Node :=
           ATN.First_Node (ATN.Subprogram_Calls (Call_Sequence));

         while Present (List_Node) loop
            New_Subinstance := Instantiate_Subprogram_Call (Instance_Root,
                                                       List_Node);
   ...
   is
      ...
      New_Instance       : constant Node_Id := New_Node
        (K_Call_Instance, ATN.Loc (Subprogram_Call));
      Container   : constant Node_Id := Container_Component
        (ATN.Parent_Sequence (Subprogram_Call));
   ...
   begin
      AIN.Set_Identifier (New_Instance,
                          AIE.Duplicate_Identifier
                            (ATN.Identifier (Subprogram_Call)));
      ...
      then
         ...
         if ATN.Kind (ATE.Get_Referenced_Entity
                        (Entity_Ref (Subprogram_Call))) = K_Subcomponent_Access
         then
            Namespace_Model := ATN.Namespace
              (ATE.Get_Referenced_Entity (Entity_Ref (ATE.Get_Referenced_Entity
                             (Entity_Ref (Subprogram_Call)))));
         else
            Namespace_Model := ATN.Namespace
         ...
         if ATNU.Length (ATN.Path (Entity_Ref (Subprogram_Call))) > 1 then
            Owner_Data := ATN.Corresponding_Entity
              (ATN.Item
               (ATN.First_Node
                (ATN.Path
                 (Entity_Ref
            ...
            if Present (Owner_Data)
              and then (Kind (Owner_Data) = K_Component_Type
            ...
            then
               ...
               begin
                  ...
                  Append_Node_To_List
                    (Make_Node_Container (D), AIN.Path (New_Instance));
                  ...
                  while Present (F) loop
                     ...
                     if Kind (F) = K_Subprogram_Spec_Instance and then
                       Corresponding_Declaration (F) =
                       Entity (Entity_Ref (Subprogram_Call))
                     then
                        Append_Node_To_List
                          (Make_Node_Container (F), AIN.Path (New_Instance));
                     end if;
         ...
         declare
            Component : constant Node_Id := ATE.Get_Referenced_Entity
              (Entity_Ref (Subprogram_Call));
            Spg_Component : Node_Id := No_Node;
         begin
            if ATN.Kind (Component) = K_Subcomponent_Access and then
              Present (ATN.Entity (Entity_Ref (Component)))
            then
