...
package body Ocarina.Builder.AADL_BA.Actions is
   ...
   is
      ...
      pragma Assert (Kind (Action_Node) = K_Assignment_Action
   ...
   is
      ...
      pragma Assert (Kind (Range_Node) = K_Id
                       or else Kind (Range_Node) = K_Integer_Range);
   ...
   is
      ...
      pragma Assert (No (Scd_Behav_Time)
                       or else Kind (Scd_Behav_Time) = K_Behavior_Time);
      pragma Assert (Distribution /= DK_Error);

      Timed_Action : constant Node_Id := New_Node (K_Timed_Action,
                                                  Loc);
   begin
