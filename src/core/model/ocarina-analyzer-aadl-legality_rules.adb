...
package body Ocarina.Analyzer.AADL.Legality_Rules is
   ...
   function Check_Rules_In_Component_Type
     (Node  : Node_Id)
     return Boolean;
   ...
   begin
      ...
      if Kind (Feature) /= K_Feature_Group_Spec
        ...
        and then Kind (Feature) /= K_Subcomponent_Access then
         DAE (Loc      => Loc (Feature),
              Node1    => Feature,
         ...
         when AADL_V1 =>
            case Component_Categ is
               ...
               when CC_Thread
                 | CC_Thread_Group
                 | CC_Process =>
                  OK := Kind (Feature) = K_Feature_Group_Spec
                    or else Kind (Feature) = K_Port_Spec
                    or else (Kind (Feature) = K_Subcomponent_Access
                             and then Component_Category'Val
                             (Subcomponent_Category (Feature)) = CC_Data)
               ...
               when CC_Memory
                 | CC_Bus =>
                  OK := Kind (Feature) = K_Subcomponent_Access
                    and then not Is_Provided (Feature)
                    and then Component_Category'Val
                    (Subcomponent_Category (Feature)) = CC_Bus;
            ...
            case Component_Categ is
               ...
               when CC_Subprogram_Group =>
                  OK := Kind (Feature) = K_Feature_Group_Spec
                    or else (Kind (Feature) = K_Subcomponent_Access
                             and then Component_Category'Val
                             (Subcomponent_Category (Feature)) = CC_Subprogram)
                    or else (Kind (Feature) = K_Subcomponent_Access
                             and then not Is_Provided (Feature)
                             and then Component_Category'Val
                             (Subcomponent_Category (Feature)) =
               ...
               when CC_Thread_Group
                 | CC_Process =>
                  case Subcomponent_Categ is
                     when CC_Data
                       | CC_Thread
                       | CC_Thread_Group => OK := True;
                     when others => OK := False;
                  end case;

               when CC_Memory =>
                  case Subcomponent_Categ is
                     when CC_Memory => OK := True;
                     when others => OK := False;
                  end case;
   ...
   is
      pragma Assert (Present (Component)
                     and then Present (Subprogram_Call_Seq));
   ...
   begin
      ...
      if Kind (Subprogram_Call_Seq) /= K_Subprogram_Call_Sequence then
         DAE (Loc      => Loc (Subprogram_Call_Seq),
   ...
   begin
      ...
      if not Is_Empty (Features (Node)) then
         ...
         while Present (List_Node) loop
            Success := A_Component_Feature (Node, List_Node)
              and then Success;
      ...
      if not Is_Empty (Flows (Node)) then
         ...
         while Present (List_Node) loop
            Success := A_Component_Flow_Specification (Node, List_Node)
              and then Success;
            List_Node := Next_Node (List_Node);
         end loop;
      end if;
   ...
   begin
      ...
      if not Is_Empty (Subcomponents (Node)) then
         ...
         while Present (List_Node) loop
            Success := A_Component_Subcomponent (Node, List_Node)
              and then Success;
      ...
      if not Is_Empty (Ocarina.Me_AADL.AADL_Tree.Nodes.Connections (Node)) then
         ...
         while Present (List_Node) loop
            Success := A_Component_Connection (Node, List_Node)
              and then Success;
            List_Node := Next_Node (List_Node);
         end loop;
      end if;
      ...
      if not Is_Empty (Flows (Node)) then
         ...
         while Present (List_Node) loop
            Success := A_Component_Flow (Node, List_Node)
              and then Success;
            List_Node := Next_Node (List_Node);
         end loop;
      end if;
         ...
         while Present (List_Node) loop
            case Kind (List_Node) is
               when K_Component_Type =>
                  Success := Check_Rules_In_Component_Type
                    (List_Node)
                    and then Success;

               when K_Component_Implementation =>
                  Success := Check_Rules_In_Component_Implementation
                    (List_Node)
                    and then Success;

               when K_Package_Specification =>
                  if not Is_Empty (Declarations (List_Node)) then
                     Package_List_Node := First_Node
