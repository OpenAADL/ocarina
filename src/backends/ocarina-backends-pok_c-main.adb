...
package body Ocarina.Backends.POK_C.Main is
   ...
   package body Source_File is
      ...
      begin
         if ARINC653_Discipline /= Invalid and then
            ARINC653_Discipline = Priority_Based then
      ...
      begin
         ...
         begin
            if TA /= Empty_Time_Array then
               if POK_Flavor = ARINC653 then
                  Capacity := Map_Time_To_Millisecond
                     (TA (1));
               else
               ...
               Display_Error
                  ("Compute execution time not declared", Fatal => False);

               --  By default, we allocate 1 ms for thread execution.
               Capacity := CTU.Make_Literal
         ...
         if POK_Flavor = ARINC653 then
            Append_Node_To_List (Make_Variable_Address
                                (Copy_Node (Tattr)), Parameters);
         ...
         if POK_Flavor = ARINC653 then
            ...
            Append_Node_To_List
               (POK_Make_Function_Call_With_Assert
                  (RF (RE_Pok_Thread_Create), Parameters),
               Statements);
         end if;
      ...
      is
         ...
         begin
            ...
            while Present (F) loop
               ...
               if Kind (F) = K_Port_Spec_Instance and then Is_In (F)
                  and then not Is_Out (F) and then
                  ...
                  not Is_Virtual (Get_Port_By_Name (F, Current_Device)) then
                     if AIN.Is_Data (F) and then Is_Event (F) then
                           Append_Node_To_List
                              (Make_Literal
                                 (CV.New_Pointed_Char_Value
                                    (Map_Port_Name (F))), Call_Parameters);
                           ...
                           Append_Node_To_List
                              (Added_Parameter, Call_Parameters);
                           ...
                           if POK_Flavor = ARINC653 then
                              ...
                              Called_Function
                                 := RF (RE_Create_Queuing_Port);
                           else
                              Called_Function
                                 := RF (RE_Pok_Port_Queueing_Create);
                           end if;
                           ...
                           if POK_Flavor = ARINC653 then
                              Append_Node_To_List
                                 (RE (RE_Destination), Call_Parameters);
                           else
                              Append_Node_To_List
                                 (RE (RE_Pok_Port_Direction_In),
                                 Call_Parameters);
                           ...
                           if POK_Flavor = ARINC653 then
                              ...
                              Called_Function
                                 := RF (RE_Pok_Port_Sampling_Create);
                           end if;
         ...
         begin
            ...
            while Present (F) loop
               ...
               if Kind (F) = K_Port_Spec_Instance and then Is_Data (F) and then
                  Get_Connection_Pattern (F) = Inter_Process and then
                  Is_Virtual (Get_Port_By_Name (F, Current_Device)) then
                     ...
                     N := Make_Call_Profile
                        (RF (RE_Pok_Port_Virtual_Create), Call_Parameters);
                     Append_Node_To_List (N, Statements);
      ...
      begin
                  ...
                  if Is_Protected_Data (Shared_Data) then
                     ...
                     if POK_Flavor = ARINC653 then
                        ...
                        Function_Call := POK_Make_Function_Call_With_Assert
                           (RF (RE_Create_Semaphore), Call_Parameters);
                     else
                        Append_Node_To_List
                           (Make_Variable_Address
                              (Make_Defining_Identifier
                        ...
                        Function_Call := POK_Make_Function_Call_With_Assert
                           (RF (RE_Pok_Sem_Create), Call_Parameters);
                     end if;
         ...
         if Use_Error_Handling then
            ...
            if POK_Flavor = ARINC653 then
               ...
               Append_Node_To_List
                  (RE (RE_Pok_Error_Handler_Worker), Call_Parameters);
         ...
         if POK_Flavor = POK then
            ...
            N := CTU.Make_Call_Profile
               (RF (RE_Set_Partition_Mode), Call_Parameters);
         end if;
         Append_Node_To_List (N, Statements);
         ...
         if Use_Error_Handling then
            ...
            N := CTU.Make_Define_Statement
               (Defining_Identifier => RE (RE_Pok_Use_Generated_Error_Handler),
            ...
            if POK_Flavor = ARINC653 then
               ...
               Used_Member := Make_Defining_Identifier
                  (MN (M_Failed_Thread));
            end if;
            Append_Node_To_List
      ...
      begin
         ...
         if Entrypoint /= No_Node then
            Init_Function := Make_Call_Profile
                  ...
                  Get_Connection_Pattern (F) = Intra_Process then
                     if AIN.Is_Data (F) and then not AIN.Is_Event (F) then
                           ...
                           if POK_Flavor = ARINC653 then
                              ...
                              Called_Function
                                 := RF (RE_Create_Blackboard);
                              Type_Used := RE (RE_Blackboard_Id_Type);
                           else
                              Called_Function
                                 := RF (RE_Pok_Blackboard_Create);
                     ...
                     elsif AIN.Is_Data (F) and then Is_Event (F)
                           and then Is_In (F) then
                           if POK_Flavor = ARINC653 then
                              Append_Node_To_List
                                 (Make_Literal
                                    (CV.New_Pointed_Char_Value
                              ...
                              Append_Node_To_List
                                 (Map_Queueing_Policy (F), Call_Parameters);
                           ...
                           Append_Node_To_List
                              (Make_Extern_Entity_Declaration
                                 (Make_Variable_Declaration
                                    ...
                                    Used_type => Type_Used)),
                              CTN.Declarations (Current_File));
                     elsif not AIN.Is_Data (F) and then Is_Event (F) and then
                        Is_In (F) then
                           if POK_Flavor = ARINC653 then
                              Append_Node_To_List
                                 (Make_Literal
      ...
      begin
         ...
         if Get_ARINC653_HM_Errors (E) /= ARINC653_Empty_Errors then
            declare
               Errors      : constant ARINC653_Errors
