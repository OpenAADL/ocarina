...
package Ocarina.Backends.POK_C.Runtime is
   ...
   subtype MR_Id is RE_Id range
      RE_Entry_Point .. RE_Period;

   --  The MR type are the potential member runtime. These identifier
   --  are used in structures, unions, etc.

   subtype ARF_Id is RE_Id range
      RE_Create_Blackboard .. RE_Raise_Application_Error;
   --  The ARF_Id type represents ARINC653 functions.

   subtype PRF_Id is RE_Id range
      RE_Pok_Thread_Sleep .. RE_Wait_For_Tasks;
   --  The PRF_Id type represents POK functions.

   subtype ART_Id is RE_Id range
      RE_Apex_Char .. RE_Return_Code_Type;
   --  The ART_Id type represents ARINC653 types.

   subtype PRT_Id is RE_Id range
      RE_Pok_Ret_T .. RE_Node_T;
   ...
   subtype RV_Id is RE_Id range
            RE_Pok_Buffers_Names .. RE_Pok_Blackboards_Names;
