   ...
   begin
      ...
      if not BANu.Is_Empty (BATN.Variables (BA)) then
         ...
         loop
            ...
            loop
               --  helping us to understand the code
               --  put_Line (T'Img);
               --  Put_Line (BATN.Kind (t)'img);
               --  wni (T);
               --  Put_Line (BATN.Kind (BATN.Classifier_Ref (P))'img);
               --  Put_Line (AIN.Kind (BATN.Classifier_Ref (P))'img);

               --  Ocarina.ME_AADL.AADL_Instances.Debug.wni
               --  (AIN.First_Node (list_id (BATN.Classifier_Ref (P))));

               --  Here for used Type, we must take the case
               --  when the type variable is any type declared
               --  in the model !!!!!
               --  In the following we suppose that the varible
               --  has a type of one of the subprogram parameters
               --
               N := Get_Instances_Of_Component_Type
                 (S,
