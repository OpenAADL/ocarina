...
--   package ATE renames Ocarina.ME_AADL.AADL_Tree.Entities;
   ...
   begin
      ...
      if Kind (Spg) = K_Component_Instance
         and then AINU.Is_Subprogram (Spg) then
   ...
   begin
      ...
      if Present (Backend_Node (Identifier (E)))
        and then Present (CTN.Type_Definition_Node
   ...
   function Map_C_Defining_Identifier
     (A : Node_Id; Is_Pointer : Boolean := False) return Node_Id is
      I, J   : Node_Id;
      Result : Node_Id;
      Name_List : List_Id;
   ...
   begin
      ...
      Result := CTU.Make_Defining_Identifier
         (To_C_Name (Name_Find), True, Is_Pointer);
   ...
   function Map_Scade_Function_Name (Subprogram : Node_Id)
      return Node_Id is
   begin
      Get_Name_String (Get_Source_Name (Subprogram));

      return CTU.Make_Defining_Identifier
         (Name_Find, C_Conversion => False);
   ...
   function Map_Scade_Struct_In (Subprogram : Node_Id)
      return Node_Id is
   begin
   ...
   function Map_Scade_Struct_Out (Subprogram : Node_Id)
      return Node_Id is
   begin
   ...
   function Map_Scade_Parameter (Parameter : Node_Id)
      return Node_Id is
   begin
   ...
   function Map_Simulink_Var (Corresponding_Feature : Node_Id)
      return Node_Id is
   ...
   begin
      ...
      Simulink_Var := Make_Member_Designator
         (Make_Defining_Identifier
            (Get_Source_Name (Corresponding_Feature), False),
         Make_Defining_Identifier (Variable_Name, False), False);
      return Simulink_Var;
   ...
   function Map_Simulink_Init_Func (Subprogram : Node_Id)
      return Node_Id is
   begin
      Get_Name_String (Get_Source_Name (Subprogram));
   ...
   function Map_Simulink_Model_Type (Subprogram : Node_Id)
      return Node_Id is
   begin
   ...
   function Map_Source_Name (Subprogram : Node_Id)
      return Node_Id is
   begin
      Set_Str_To_Name_Buffer ("");
      Get_Name_String_And_Append (Get_Source_Name (Subprogram));
   ...
   function Map_Lustre_Context_Name (Subprogram : Node_Id)
      return Node_Id is
   begin
      Set_Str_To_Name_Buffer ("");
      Get_Name_String_And_Append (Get_Source_Name (Subprogram));
   ...
   function Map_Lustre_Context_Reset (Subprogram : Node_Id)
      return Node_Id is
   begin
      Set_Str_To_Name_Buffer ("");
      Get_Name_String_And_Append (Get_Source_Name (Subprogram));
   ...
   function Map_Lustre_Context_Init (Subprogram : Node_Id)
      return Node_Id is
   begin
      Set_Str_To_Name_Buffer ("");
      Get_Name_String_And_Append (Get_Source_Name (Subprogram));
   ...
   function Map_Lustre_Context_Type (Subprogram : Node_Id)
      return Node_Id is
   begin
   ...
   function Map_Lustre_Step_Name (Subprogram : Node_Id)
      return Node_Id is
   begin
      Set_Str_To_Name_Buffer ("");
      Get_Name_String_And_Append (Get_Source_Name (Subprogram));
   ...
   function Map_Esterel_Header (Subprogram : Node_Id)
      return Node_Id is
   begin
      Set_Str_To_Name_Buffer ("");
   ...
   function Map_C_Marshaller_Subprogram
     (A             : Node_Id;
      ...
      Is_Request    : Boolean := False) return Node_Id is
      I             : Node_Id;
      Result        : Node_Id;
      N             : Name_Id;
      C_Name      : Name_Id;
      ...
      case Get_Subprogram_Kind (S) is
         ...
         when Subprogram_Opaque_C =>
            ...
            then
               ...
               while Present (Feature) loop
                  if Kind (Feature) = K_Subcomponent_Access_Instance
                    and then
                    Is_Protected_Data (Corresponding_Instance (Feature))
                  then
                     ...
                     Append_Node_To_List (Make_Defining_Identifier
                        (Map_Associated_Locking_Entity_Name
                           (Get_Accessed_Data (Feature))), Call_Parameters);
                     ...
                     if POK_Flavor = ARINC653 then
                        ...
                        Function_Call := POK_Make_Function_Call_With_Assert
                           (RF (RE_Pok_Sem_Wait), Call_Parameters);
                     end if;
            ...
            then
               ...
               while Present (Feature) loop
                  if Kind (Feature) = K_Subcomponent_Access_Instance and then
                     Is_Protected_Data (Corresponding_Instance (Feature)) then
                     Call_Parameters := New_List (CTN.K_Parameter_List);
                     ...
                     if POK_Flavor = ARINC653 then
                        ...
                        Function_Call := POK_Make_Function_Call_With_Assert
                           (RF (RE_Pok_Sem_Signal), Call_Parameters);
                     end if;
                     Append_Node_To_List (Function_Call, Statements);
                  end if;
                  Feature := AIN.Next_Node (Feature);
         ...
         when Subprogram_Opaque_Ada_95 =>
            ...
            N := Make_Call_Profile
              (Make_Defining_Identifier
                 (Get_Source_Name (S),
                  Ada_Conversion => True), Call_Profile);
         ...
         when Subprogram_Simulink =>
            Add_Include
               (Make_Include_Clause
                  (Make_Defining_Identifier
                     (Get_Source_Name (S), False), False), True);
               ...
               while Present (P) loop
                  if AIN.Is_In (P) then
                     ...
                     if Get_Data_Representation
                        (Corresponding_Instance (P)) = Data_Array then
                        Append_Node_To_List
                           (Make_Call_Profile
                              (RE (RE_Copy_Array),
                              Make_List_Id
                        ...
                        Append_Node_To_List
                           (Make_Expression
                              (Left_Assign, Op_Equal, Right_Assign),
                           Statements);
                     end if;
                  end if;
         ...
         when Subprogram_Scade =>
            Add_Include
               (Make_Include_Clause
                  (Map_Scade_Function_Name (S)), Preserve_Case => True);
            ...
            if not AINU.Is_Empty (Features (S)) then
               ...
               while Present (P) loop
                  if AIN.Is_In (P) then
                     ...
                     Left_Assign := Make_Member_Designator
                     ...
                     if T /= No_Node and then
                        CTN.Kind (T) = CTN.K_Array_Declaration then
                           Call_Profile
                              := New_List (CTN.K_Parameter_Profile);
                           ...
                           Append_Node_To_List
                              (Get_Data_Size (Z), Call_Profile);

                           N := CTU.Make_Call_Profile
                                 (RE (RE_Copy_Array), Call_Profile);
                           Append_Node_To_List (N, Statements);
                     else
                        Append_Node_To_List
                           (Make_Expression
            ...
            Append_Node_To_List
               (CTU.Make_Call_Profile
                  (Map_Scade_Function_Name (S), Call_Profile),
               Statements);
            ...
            if not AINU.Is_Empty (Features (S)) then
               ...
               while Present (P) loop
                  if AIN.Is_Out (P) then
                     ...
                     Left_Assign := Map_C_Defining_Identifier (P);
                     Right_Assign := Make_Member_Designator
                        (Map_Scade_Parameter (P),
                        Make_Defining_Identifier (VN (V_Out)));
         ...
         when Subprogram_Lustre =>
            Add_Include
               (Make_Include_Clause
                  (Map_Source_Name (S)), Preserve_Case => True);
               ...
               while Present (P) loop
                  if AIN.Is_Out (P) then
                     ...
                     begin
                        ...
                        Spec := CTU.Make_Function_Specification
                           (Defining_Identifier =>
                        ...
                        Append_Node_To_List
                           (Make_Expression
                              (Left_Expr =>
                                 Map_C_Defining_Identifier (P, True),
                              Operator => Op_Equal,
                              Right_Expr =>
                                 Map_Lustre_Temp_Var (S, P)),
                            Statements);
         ...
         when Subprogram_Esterel =>
            Add_Include
               (Make_Include_Clause
                  (Map_Esterel_Header (S)), Preserve_Case => True);
         ...
         when Subprogram_Pure_Call_Sequence =>
            ...
            CTU.Handle_Call_Sequence
              (S, AIN.First_Node (AIN.Calls (S)), Declarations, Statements);
   ...
   begin
      ...
      if Get_Connection_Pattern (E) = Inter_Process or else
         Get_Port_By_Name (E, Containing_Component) /= No_Node then
         if Get_Category_Of_Component
               (Parent_Subcomponent
                  (Parent_Component (E))) = CC_Thread then
            if Is_In (E) and then not Is_Out (E) then
               if AIN.Sources (E) /= No_List and then
                  not AINU.Is_Empty (AIN.Sources (E)) then
               ...
               if AIN.Destinations (E) /= No_List and then
                  not AINU.Is_Empty (AIN.Destinations (E)) then
            ...
            (Parent_Subcomponent (Parent_Component (E))) = CC_Process then
            F := E;
         elsif Get_Category_Of_Component
            (Parent_Subcomponent (Parent_Component (E))) = CC_Device then
            F := E;
         end if;
   ...
   begin
      ...
      Get_Name_String
         (Display_Name
            (Identifier (Parent_Subcomponent (Processor))));
      N := Name_Find;
      return N;
   end Map_Node_Name;
   ...
   begin
      case Action is
         when POK_Action_Ignore =>
            N := Make_Call_Profile
               (RE (RE_Pok_Error_Ignore), No_List);

         when POK_Action_Kernel_Stop =>
            N := Make_Call_Profile
               (RE (RE_Pok_Kernel_Stop), No_List);

         when POK_Action_Kernel_Restart =>
            N := Make_Call_Profile
               (RE (RE_Pok_Kernel_Restart), No_List);
         ...
         when POK_Action_Nothing =>
            if Kernel_Level then
               N := Make_Call_Profile
                  (RE (RE_Pok_Error_Kernel_CallBack), No_List);
            else
               N := Make_Call_Profile
                  (RE (RE_Pok_Error_Partition_Callback),
                  Make_List_Id
                     (Make_Literal
                        (New_Int_Value (Partition_Id, 1, 10))));
            end if;
   ...
   function Map_POK_Error (Error : Supported_POK_Error)
      return Node_Id is
   begin
      case Error is
   ...
   begin
      ...
      while Present (Tmp_Node) loop
         ...
         if Handled_Kind = Sending then
            Subprogram := Look_For_Subcomponent_In_Declarative
               (Implementation, "send");
         else
            Subprogram := Look_For_Subcomponent_In_Declarative
         ...
         if Property_Node /= No_Node then
            ...
            --  (data_to_marshall, size, data_marshalled, size)
               ...
               if Get_Data_Representation
                  (Corresponding_Instance (Port)) = Data_Array then
                  Append_Node_To_List
                     (Make_Defining_Identifier
                      (Map_Port_Data
                       (Port, Containing_Component)),
                      Call_Parameters);
               else
                  Append_Node_To_List
                  (Make_Variable_Address
               ...
               Append_Node_To_List (Get_Data_Size
               ...
               Append_Node_To_List
                  (Make_Variable_Address (Make_Defining_Identifier
                     (Map_Port_Data_With_Virtual_Bus
                        (Port, Virtual_Bus, Containing_Component))),
                   Call_Parameters);
            ...
            Function_Call := Make_Call_Profile
               (Make_Defining_Identifier
                (Get_String_Of_Property_Value
                 (AAN.Single_Value (Property_Node))),
                Call_Parameters);
            ...
            New_Data := Make_Defining_Identifier
               (Map_Port_Data_With_Virtual_Bus
                  (Port, Virtual_Bus, Containing_Component));

            New_Size := Make_Defining_Identifier
               (Map_Port_Var_Length_With_Virtual_Bus
                  (Port, Virtual_Bus, Containing_Component));
   ...
   begin
      ...
      if Instance_Type /= No_Node and then
         Is_Defined_Property (Instance_Type, "type_source_name") then
         return Make_Defining_Identifier
            (To_C_Name
            (Get_String_Property (Instance_Type, "type_source_name")));
      end if;

      return Map_C_Data_Type_Designator
         (Corresponding_Instance (Port));
   ...
   begin
      ...
      while Present (Tmp_Node) loop
         ...
         Property_Node := Look_For_Property_In_Declarative
            (Virtual_Bus, "pok::protocol");

         if Property_Node /= No_Node then
            if Get_Enumeration_Of_Property_Value
               (AAN.Single_Value (Property_Node)) = "ceasar" then
               CTU.Append_Node_To_List
                  (CTU.Make_Define_Statement
                     (Defining_Identifier =>
                        RE (RE_Pok_Needs_Protocols_Ceasar),
                      Value =>
                        CTU.Make_Literal (New_Int_Value (1, 1, 10))),
         ...
         if Property_Node /= No_Node then
            if Get_Enumeration_Of_Property_Value
               (AAN.Single_Value (Property_Node)) = "des" then
               CTU.Append_Node_To_List
                  (CTU.Make_Define_Statement
                     (Defining_Identifier =>
                        RE (RE_Pok_Needs_Protocols_Des),
                      Value =>
                        CTU.Make_Literal (New_Int_Value (1, 1, 10))),
                  CTN.Declarations (CTU.Current_File));
            end if;
         ...
         if Property_Node /= No_Node then
            if Get_Enumeration_Of_Property_Value
               (AAN.Single_Value (Property_Node)) = "blowfish" then
               CTU.Append_Node_To_List
                  (CTU.Make_Define_Statement
                     (Defining_Identifier =>
