...
package body Ocarina.FE_AADL_BA.Lexer is
   ...
   --  This suffix is used to avoid conflict when we initialize
   --  tokens with multiple parsers
   ...
   begin
      ...
      --  Add the prefix for avoid crash in token init
      Add_Str_To_Name_Buffer (Prefix);
      ...
      if B in BA_First_Reserved_Word_Pos .. BA_Last_Reserved_Word_Pos then
         if Token_Name = BA_Token_Image (BA_Token_Type'Val (B)) then
            Token := BA_Token_Type'Val (B);
   ...
   procedure Scan_Raw_Text (T : BA_Token_Type) is
         ...
         case Buffer (Token_Location.Scan) is
            ...
            when '!' =>      --  '!' or '!=' or '!>' or '!<'
               ...
               if Buffer (Token_Location.Scan) = '=' then
                  ...
                  Token_Location.Scan := Token_Location.Scan + 1;
                  Token := T_Exclamation_Greater;
               elsif Buffer (Token_Location.Scan) = '<' then
                  Token_Location.Scan := Token_Location.Scan + 1;
                  Token := T_Exclamation_Lesser;
               else
                  Token := T_Exclamation;
               end if;
   ...
   begin
      ...
      while Token /= T_EOF loop
         ...
         if Token in BA_Opening_Delimiter then
            Braces := Braces + 1;
         elsif Token in BA_Closing_Delimiter then
            Braces := Braces - 1;
         end if;
   ...
   procedure Skip_Tokens (Delimiters : BA_Token_List_Type) is
   begin
